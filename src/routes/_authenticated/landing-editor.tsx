...
function LandingEditorPage() {
  ...
  const addElement = (kind: string) => {
    setContent((current) => {
      ...
      if (kind === "plan") return { ...current, plans: { ...current.plans, items: [...current.plans.items, { id: `plan_${Date.now()}`, priceId: "", name: "Novo plano", price: "R$ 0", priceUsd: "$0", period: "pagamento único", amountCents: 0, amountCentsUsd: 0, credits: 0, description: "Descrição do plano", features: ["Benefício"], active: false, highlight: false, free: false, cta: "Comprar créditos" }] } };

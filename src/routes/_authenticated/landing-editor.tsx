          ...
          {selection ? (
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[["feature", "Recurso"], ["benefit", "Benefício"], ["stat", "Número"], ["step", "Passo"], ["audience", "Público"], ["faq", "Pergunta"], ["badge", "Selo"]].map(([kind, label]) => <Button key={kind} variant="outline" size="sm" onClick={() => addElement(kind)}><Plus className="size-3.5" />{label}</Button>)}
              </div>
              <p className="mb-2 mt-6 text-xs font-bold">Ordem das seções</p>
              <div className="space-y-1">{content.sections.map((section, index) => <div key={section} draggable onDragStart={(event) => event.dataTransfer.setData("text/section-index", String(index))} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const from = Number(event.dataTransfer.getData("text/section-index")); if (!Number.isFinite(from) || from === index) return; setContent((current) => { const sections = [...current.sections]; const [moved] = sections.splice(from, 1); if (!moved) return current; sections.splice(index, 0, moved); return { ...current, sections }; }); }} className="flex cursor-grab items-center justify-between border border-border px-3 py-2 text-xs font-bold"><span>{section}</span><div className="flex"><Button size="icon" variant="ghost" disabled={index === 0} onClick={() => setContent((current) => { const sections = [...current.sections]; [sections[index - 1], sections[index]] = [sections[index], sections[index - 1]]; return { ...current, sections }; })}><ArrowUp className="size-3.5" /></Button><Button size="icon" variant="ghost" disabled={index === content.sections.length - 1} onClick={() => setContent((current) => { const sections = [...current.sections]; [sections[index], sections[index + 1]] = [sections[index + 1], sections[index]]; return { ...current, sections }; })}><ArrowDown className="size-3.5" /></Button></div></div>)}</div>
            </div>
          )}
        </aside>
      </div>
